...
namespace TapoDevices
{
    class KlapCipher
    {
        ...
        public byte[] Encrypt(byte[] plainText)
        {
            ...
            var signature = SHA256.Create().ComputeHash(dataToHash);
        ...
        private static byte[] KeyDerive(byte[] localSeed, byte[] remoteSeed, byte[] userHash)
        {
            ...
            var key = SHA256.Create().ComputeHash(payload.ToArray());
        ...
        private byte[] IvDerive(byte[] localSeed, byte[] remoteSeed, byte[] userHash)
        {
            ...
            var fulliv = SHA256.Create().ComputeHash(payload.ToArray());
        ...
        private static byte[] SigDerive(byte[] localSeed, byte[] remoteSeed, byte[] userHash)
        {
            ...
            var sig = SHA256.Create().ComputeHash(payload.ToArray());
