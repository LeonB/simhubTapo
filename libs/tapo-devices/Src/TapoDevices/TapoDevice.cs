...
namespace TapoDevices
{
    ...
    public class TapoDevice
    {
        ...
        public async Task ConnectAsync()
        {
            ...
            var localSeedAuthHash = SHA256.Create().ComputeHash(ls.ToArray());
            ...
            if (Utils.ByteArraysEqual(localSeedAuthHash, serverHash))
            {
                ...
                var payload = SHA256.Create().ComputeHash(handshake2.ToArray());
        ...
        {
            var usernameHash = SHA1.Create().ComputeHash(Encoding.UTF8.GetBytes(username));
            var passwordHash = SHA1.Create().ComputeHash(Encoding.UTF8.GetBytes(password));
            var authHashSource = new List<byte>();
            authHashSource.AddRange(usernameHash);
            authHashSource.AddRange(passwordHash);

            return SHA256.Create().ComputeHash(authHashSource.ToArray());
        }
        ...
        public async Task ConnectOldAsync()
        {
            var key = RSA.Create();
            key.KeySize = 1024;
            var publicKey = Compat.ExportSubjectPublicKeyInfo(key); // TODO: replacement for .NET Standard 2.0
            ...
            var hashedUsername = SHA1.Create().ComputeHash(Encoding.UTF8.GetBytes(this.username));
